...
<div class="container" id="actualizacion-contador">
    ...
    <div class="progress-bar">
        <div class="step {{ $deposit->status == 'Pending' ? 'active1' : '' }}">PENDIENTE</div>
        <div class="step {{ $deposit->status == 'Processing' ? 'active1' : '' }}">PROCESANDO</div>
        <div class="step {{ $deposit->status == 'Processed' ? 'active1' : '' }}">COMPLETO</div>
    </div>

    <div class="loading-spinner" id="loading-spinner"
        style="display: {{ $deposit->status == 'Processed' ? 'none' : 'block' }};"></div>
    <h2>ESTADO: <span id="estado">{{ $deposit->status }}</span></h2>
...
<script>
    setInterval(function () {
        ...
        fetch(protocolo + '//' + dominio + '/dashboard/fetch-deposito/{{ $deposit->id }}', {
            method: 'GET',
            mode: 'no-cors',
        })
            ...
            .then(data => { 
                ...
                if (data.status === 'Processing') {
                    ...
                    document.getElementById('procedado').innerHTML = '<img class="imgPdf" src="{{ asset('assets/img/pdf.svg') }}" /><br><button onclick="window.open(\'{{ route('depositofactura', $deposit->id) }}\', \'_blank\')" class="button">IMPRIMIR FACTURA</button>';
