...
<div class="container" id="actualizacion-contador">
    ...
    <div class="progress-bar">
        <div class="step {{ $withdrawal->status == 'Pending' ? 'active1' : '' }}">PENDIENTE</div>
        <div class="step {{ $withdrawal->status == 'Processing' ? 'active1' : '' }}">PROCESANDO</div>
        <div class="step {{ $withdrawal->status == 'Processed' ? 'active1' : '' }}">COMPLETO</div>
    </div>

    <div class="loading-spinner" id="loading-spinner"
        style="display: {{ $withdrawal->status == 'Processed' ? 'none' : 'block' }};"></div>
    <h2>ESTADO: <span id="estado">{{ $withdrawal->status }}</span></h2>
...
<script>
    setInterval(function () {
        ...
        fetch(protocolo + '//' + dominio + '/dashboard/fetch-retiro/{{ $withdrawal->id }}', {
            method: 'GET',
            mode: 'no-cors',
        })
            ...
            .then(data => { 
                ...
                if (data.status === 'Processing') {
                    ...
                    document.getElementById('procedado').innerHTML = '<img class="imgPdf" src="{{ asset('assets/img/pdf.svg') }}" /><br><button onclick="window.open(\'{{ route('retirofactura', $withdrawal->id) }}\', \'_blank\')" class="button">IMPRIMIR FACTURA</button>';
